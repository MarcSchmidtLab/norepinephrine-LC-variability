function optimize_scheme_CallbackOLD(hObject, eventdata, handles)
...
for templateind = 1:templatenm
    ...
    while cont == 1
        ...
        if length(mininds) > 1
            temp1{metaind} = mkTemplateDTW(clipstrct.specarr(tempinds{templateind}(mininds)));
        else
            temp1(metaind) = clipstrct.specarr(tempinds{templateind}(mininds));
        end
...
for templateind = 1:templatenm
    ...
    for threshind = 1:length(threshvc)
       ...
       errvc(threshind) = (falsepos + falseneg) / maxnm; 
    end
