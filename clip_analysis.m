...
for templateindind = 1:length(template_selectinds)
    ...
    for matind = 1:matnm
        ...
        for matclipind = 1:matclipnm
            ...
            [s_dims,temp_var] = size(temp_file);
            %s_dims = wavread(flnm,'size');
            samp2 = min(samp2,max(s_dims));
            temp_file_slice = temp_file(samp1:samp2);
            signal = filter(b,a,temp_file_slice);
...
for templateindind = 1:length(template_selectinds)
    ...
    for sampleind = 1:N
        ...
        %disp(flnm)
        temp_file = wavread(flnm);
        [s_dim, temp_var] = size(temp_file);
        %s_dim = wavread(flnm,'size'); - can't use wavread's built-in
        %functions for size and slice for some reason
