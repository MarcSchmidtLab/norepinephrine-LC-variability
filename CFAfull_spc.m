function [W,psi,phi,sigma,omega,iter,n_fail,logp,Chat,W0,psi0,phi0,sigma0,omega0] = CFAfull_spc(X,Q,initN,m,D,R,waitbaropt)
...
if waitbaropt
    h = waitbar(0/length(inds),'Calculating top 5th FA parameters');
end
...
for i = 1:length(inds)
    ...
    [Wtmp,sigmatmp,psitmp,phitmp,omegatmp,itertmp,degflg,z,u] = CFA_full(X,Q,m,W0tmp,sigma0tmp,psi0tmp,phi0tmp,omega0tmp,D,R,0.01);
    ...
    if waitbaropt
        h = waitbar(i/length(inds),h,'Calculating top 5th FA parameters');
    end
