...
h_spectral_templates = uimenu(h_filemenu,'Label','Spectral templates','Tag','menu_spectral_templates');
uimenu(h_spectral_templates,'Label','Save','Tag','save_spectral_templates','Callback','1');
...
uimenu(h_analysis,'Label','Export spectral data','Tag','export_spectral_data','Callback','1');
...
objstrings = {'Ex/include syls','Link syls','Max gap','200','Trans matrix','Sequences','Min seq num','50',...
...
'Parent',h_mainfig,...
'Units','normalized',...
'Position',[.01 .375 .08 .15],...
'Tag','spectral_panel');

objstrings = {'Template feat.','Set features','Spec var','N','200','Seq spec var','Seq spec feat'};

objtags = {'pick_spec_features','set_spec_features','spec_dist','N_lab','N','seq_spec_var','seq_spec_feat'};

objstyles = {'pushbutton','pushbutton','pushbutton','text','edit','pushbutton','pushbutton'};

put_uicontrol(h_spectralpanel,objstrings,objtags,objstyles);
...
'Parent',h_mainfig,...
'Units','normalized',...
'Position',[.01 .05 .08 .325],...
'Tag','timing_panel');

objstrings = {'Timing template','DTW maps','Template feat.','Set features','Load seq timing',...
    'DTW seq timing','N','300','Factor anl','m','1'};

objtags = {'generate_timing_template','compile_dtw','pick_timing_features','set_timing_features','load_seq_timing',...
    'seq_timing','N_timing_lab','N_timing','factor_analysis','init_cond_lab','fa_m'};

objstyles = repmat({'pushbutton'},1,6);
objstyles{end+1} = 'text';
objstyles{end+1} = 'edit';
objstyles{end+1} = 'pushbutton';
objstyles{end+1} = 'text';
objstyles{end+1} = 'edit';
