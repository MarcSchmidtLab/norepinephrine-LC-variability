function transition_matrix_CallbackOLD2(hObject, eventdata, handles)
...
silinds = find(seqstrct.vc==silind);
...
for order = length(entvc):-1:1    
    ...
    rowlabs = [];
    for orderind = 1:order
        rowlabs = [rowlabs;labsu{rowinds(orderind,:)}];
    end
    ...
    rowlabs_cont = rowlabs(:,cont_rowinds);
    ...
    if order == pltorder
        ...
        rowlabs_plt = rowlabs;
        colabs_plt = colabs;
        rowlabs_cont_plt = rowlabs_cont;
        colabs_cont_plt = colabs_cont;
